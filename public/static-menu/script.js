...
let activeCategory = menuCategories[0].id;

// Render Category Tabs
function renderCategoryTabs() {
  categoryTabs.innerHTML = menuCategories.map(category => `
    <button 
      class="category-tab ${category.id === activeCategory ? 'active' : ''}" 
...
function renderMenuSections() {
  menuSections.innerHTML = menuCategories.map(category => `
    <section class="menu-section" id="${category.id}">
      <h2 class="section-title">${category.name}</h2>
      <div class="menu-grid">
        ${category.items.map(item => `
          <div class="menu-card" data-item='${JSON.stringify(item)}'>
            ...
            <div class="card-content">
              ...
              <span class="view-3d-hint">View in 3D →</span>
            </div>
          </div>
        `).join('')}
      </div>
...
function openModal(item) {
  ...
  // Set model source
  const modelSrc = item.modelUrl || defaultModelUrl;
